...
SELECT 
    m.Name,
    COUNT(b.BorrowID) AS SoSachMuon
FROM Members m
JOIN Borrowings b
    ON m.MemberID = b.MemberID
GROUP BY m.MemberID, m.Name
...
SELECT TOP 1
    m.Name,
    SUM(DATEDIFF(DAY, b.BorrowDate, b.ReturnDate)) AS TongSoNgayGiu
FROM Members m
JOIN Borrowings b
    ON m.MemberID = b.MemberID
WHERE b.ReturnDate IS NOT NULL
GROUP BY m.MemberID, m.Name
ORDER BY TongSoNgayGiu DESC;

SELECT 
    m.Name,
    COUNT(b.BorrowID) AS TongSoSachMuon,
    SUM(
        CASE 
            WHEN b.ReturnDate IS NULL THEN 1
            ELSE 0
        END
    ) AS SoSachChuaTra
FROM Members m
